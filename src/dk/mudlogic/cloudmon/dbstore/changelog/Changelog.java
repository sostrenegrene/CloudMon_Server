package dk.mudlogic.cloudmon.dbstore.changelog;
...
public class Changelog {
    ...
    public int has_changed(int command_id,String current_status,int result_hash) {
        ...
        try {
            ...
            if ( last_status_hash != current_status_hash ) {
                log.warning("("+command_id+")Status changed [NEW STATUS]");
            ...
            else if ( ( Boolean.parseBoolean(current_status) == true ) && (result_hash != last_result_hash) ) {
                log.warning("("+command_id+")Status changed [NEW RESULT DATA] ");
                log.warning("Last Result Hash: " + last_result_hash + " Current Result Hash: " + current_status_hash + " Last Status Hash: " + last_result_hash );
    ...
    private void update_status(int command_id,int result_hash) {
    ...
    public boolean status(int client_id,int command_id,String status,int result_hash,int changelog_type) {
        ...
        if ( changelog_type == Changelog.NEW_DATA ) {
            update_status(command_id,result_hash);
